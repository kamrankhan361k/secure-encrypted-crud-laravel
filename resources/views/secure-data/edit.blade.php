...
@section('title', 'Edit Secure Record')
    ...
    <div class="col-md-10">
        <div class="card">
            <div class="card-header">
                <h4 class="mb-0">
                    <i class="fas fa-edit"></i> Edit Secure Record #{{ $record->id }}
                </h4>
                <small class="text-muted">All data is encrypted using multi-layer AES-256 encryption</small>
            </div>
            <div class="card-body">
                <form action="{{ route('secure-data.update', $record->id) }}" method="POST" id="secureForm">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="phone" name="phone"
                                       value="{{ old('phone', $record->phone) }}" required maxlength="20">
                                @error('phone')
                                    <div class="text-danger small">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="credit_card" name="credit_card"
                                       value="{{ old('credit_card', $record->credit_card) }}" required maxlength="19">
                                @error('credit_card')
                                    <div class="text-danger small">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label for="social_security_number" class="form-label">SSN *</label>
                                <input type="text" class="form-control" id="social_security_number"
                                       name="social_security_number"
                                       value="{{ old('social_security_number', $record->social_security_number) }}"
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="security_level" name="security_level" required>
                                    ...
                                    @foreach($securityLevels as $value => $label)
                                        <option value="{{ $value }}"
                                            {{ old('security_level', $record->security_level) == $value ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="address" name="address" rows="3"
                                  required maxlength="500">{{ old('address', $record->address) }}</textarea>
                        @error('address')
                            <div class="text-danger small">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="medical_info" name="medical_info"
                                          rows="4" maxlength="1000">{{ old('medical_info', $record->medical_info) }}</textarea>
                                @error('medical_info')
                                    <div class="text-danger small">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="financial_info" name="financial_info"
                                          rows="4" maxlength="1000">{{ old('financial_info', $record->financial_info) }}</textarea>
                    ...
                    <div class="alert alert-info">
                        <h5><i class="fas fa-info-circle"></i> Update Information</h5>
                        <p class="mb-0">When you update this record, all sensitive data will be re-encrypted with the latest security parameters.</p>
                    </div>

                    <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                        <button type="submit" class="btn btn-primary btn-lg">
                            <i class="fas fa-save"></i> Update Secure Record
                        </button>
                        <a href="{{ route('secure-data.show', $record->id) }}" class="btn btn-secondary btn-lg">
